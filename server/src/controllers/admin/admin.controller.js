...
export const loginadmin = async (req, res) => {
  ...
  try {
    ...
    if (!checkemail) {
      return res
        .status(400)
        .json({ message: "Admin does not exist", status: false });
    }
    //check password
    const ispaawordcorrect = await bcrypt.compare(
      password,
      checkemail.password
    );
    if (!ispaawordcorrect) {
    ...
    const token = jwt.sign({ subid: checkemail._id }, process.env.secretkey, {
      expiresIn: "2m",
    });
    const refreshtoken = jwt.sign(
      { subid: checkemail._id },
      process.env.rfsecretkey,
      { expiresIn: "30d" }
    );
    ...
    return res
      ...
      .json({
        ...
        token: token,
        twostep: checkemail?.twoFactorEnabled,
      });
  } catch (err) {
    console.log(err);
    return res
...
export const refreshtoken = async (req, res) => {
  ...
  try {
    ...
    const newtoken = jwt.sign({ subid: decoded.subid }, process.env.secretkey, {
      expiresIn: "2m",
    });
    return res
      .status(200)
      .json({ message: "New token generated", status: true, token: newtoken });
  } catch (err) {
    console.log(err);
    return res
      .status(401)
...
export const admininfo = async (req, res) => {
  ...
  try {
    //check the details
    const admininfo = await Admin.findById(id).select(
  ...
  try {
    ...
    if (check) {
      return res
        .status(200)
        .json({
          message: "Check two step",
          twostep: check?.twoFactorEnabled,
          status: true,
        });
    }
...
export const verifylogin2fa = async (req, res) => {
  ...
  try {
    ...
    const verified = speakeasy.totp.verify({
      secret: user.twoFactorSecret, // PERMANENT SECRET
      encoding: "base32",
      token: otp,
      window: 1, // ±30 sec tolerance
    });
    ...
    const accessToken = jwt.sign({ subid: user._id }, process.env.secretkey, {
      expiresIn: "2m",
    });
    return res.json({
      success: true,
